...
namespace MFS.Repositorys
{
    public class StoreRepository : RepositoryBase<Store>
    {
        ...
        {
            if (sc == StoreClass.全部)
                return LoadPageList(1,30,out int count,true, false, s => s.StoreClass == StoreClass.存储仓 || s.StoreClass == StoreClass.销售仓).OrderBy(s => s.Name).ToList();
            else
                return LoadPageList(1, 30, out int count, true, false, s => s.StoreClass == sc).OrderBy(s => s.Name).ToList();
        }
    }
}
